...
export default function TransportsList() {
  ...
  // Fetch booking counts per transport name from itinerary_rows (stored as contracts)
  useEffect(() => {
    ...
    supabase.from('itinerary_rows').select('rows').then(({ data }) => {
      ...
      const counts = {}
      data.forEach((record) => {
        const contracts = record.rows?.contracts || []
        contracts.forEach((c) => {
  ...
  return (
    <>
      <div className="page-header">
        <h2>Transport</h2>
        <button className="btn btn-success" onClick={() => setModalItem({})}>+ Add Transport</button>
      </div>
          ...
          {loadingItems ? (
            [1,2,3,4,5].map((n) => (
              <div key={n} className="hl-card hl-card-skel">
                ...
                <div className="hl-card-prices"><div className="skel" style={{ height: 14, width: 120, borderRadius: 4 }} /></div>
            ...
            <div className="hl-empty">No transport items match your filters.</div>
          ) : (
            filtered.map((item) => {
              ...
              <div
                key={item.id}
                className="hl-card"
                onClick={() => navigate(`/transports/${slugify(item.name)}`)}
              >
                <div className="hl-card-main">
                ...
                <div className="hl-card-prices">
                  ...
                  {item.group_price != null && (
                    <span style={{ marginLeft: '0.5rem', color: 'var(--color-text-light)', fontSize: '0.8rem' }}>
                      · grp {fmtPrice(item.group_price)}
                    </span>
                  )}
                  {item.capacity != null && (
                    <span style={{ marginLeft: '0.5rem', color: 'var(--color-text-light)', fontSize: '0.8rem' }}>· {item.capacity} pax</span>
                  )}
                  {item.price_unit && (
